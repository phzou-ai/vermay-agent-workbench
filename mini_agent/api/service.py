...
class AgentService:
    def __init__(
        ...
        *,
        ...
        self._execution_locks = TaskExecutionLocks()
...
class TaskExecutionService:
    def __init__(self, *, max_workers: int = 4) -> None:
        ...
        self._executor = ThreadPoolExecutor(max_workers=max_workers, thread_name_prefix="mini-agent-task")
...
class TaskExecutionLocks:
    ...
    def acquire(self, task_id: str, *, blocking: bool = False) -> Iterator[None]:
        ...
        if not acquired:
            raise SessionConflictError(f"task is already running: {task_id}")
        try:
            yield
        finally:
            lock.release()
