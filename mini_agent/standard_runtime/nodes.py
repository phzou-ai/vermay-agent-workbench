...
def check_permission_node(components: StandardGraphComponents):
    def node(state: StandardAgentState) -> dict:
        loop_index = state["loop_index"]
...
def reject_tool_node(components: StandardGraphComponents):
    def node(state: StandardAgentState) -> dict:
        loop_index = state["loop_index"]
        ...
        if approval.get("approved") is False:
            ...
            _log_trace(components, "standard_tool_rejected", {"loop": loop_index, "reason": reason})
            _emit_progress(components, loop_index, "final_answer")
        ...
        _log_trace(components, "standard_tool_rejected", {"loop": loop_index, "reason": reason, "status": status})
        _emit_progress(components, loop_index, "final_answer")
        return {"final_answer": final_answer}
...
def approval_required_node(components: StandardGraphComponents):
    def node(state: StandardAgentState) -> dict:
        loop_index = state["loop_index"]
        permission = state.get("permission") or {}
        ...
        _log_trace(components, "standard_approval_resumed", {"loop": loop_index, "approval": approval})
...
def record_tool_messages_node(components: StandardGraphComponents):
    def node(state: StandardAgentState) -> dict:
        loop_index = state["loop_index"]
        ...
        for message in tool_messages:
            ...
            _log_trace(components, "standard_tool_message", {"loop": loop_index, **payload})
        return {}

    return node


def increment_loop_node(_: StandardGraphComponents):
    def node(state: StandardAgentState) -> dict:
        return {
...
def max_loops_node(components: StandardGraphComponents):
    def node(state: StandardAgentState) -> dict:
...
def _emit_permission(
    components: StandardGraphComponents,
    loop_index: int,
...
def _emit_tool_execute_start(
    ...
    for raw_tool_call in tool_calls:
        ...
        _log_trace(components, "standard_tool_execute_start", {"loop": loop_index, "tool_call": payload})
...
def _log_trace(components: StandardGraphComponents, event_type: str, payload: dict[str, Any]) -> None:
